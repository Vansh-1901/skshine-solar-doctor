...
export const Route = createFileRoute("/_authenticated/admin/gallery")({
  head: () => ({
    meta: [{ title: "Admin · Gallery | SK Shine" }, { name: "robots", content: "noindex" }],
  }),
...
function AdminGallery() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery({ queryKey: ["gallery"], queryFn: () => list() });
  const [category, setCategory] = useState<"residential" | "commercial" | "industrial">(
    "residential",
  );
  ...
  async function onUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      for (const file of files) {
        ...
        const {
          data: { publicUrl },
        } = supabase.storage.from("project-gallery").getPublicUrl(path);
      ...
      <div className="container mx-auto max-w-6xl">
        <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Upload new project photos — they appear instantly on the website.
            </p>
          </div>
          <button
            onClick={signOut}
            className="px-4 py-2 rounded-full border border-border bg-card text-sm inline-flex items-center gap-2"
          >
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 text-muted-foreground text-sm">
            No uploaded photos yet. The website also shows seed photos until you upload your own.
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {data.map((g) => (
              <div
                key={g.id}
                className="group relative rounded-2xl overflow-hidden bg-card border border-border shadow-soft"
              >
                <img
                  src={g.url}
                  alt={g.caption ?? ""}
                ...
                />
                <div className="absolute top-2 left-2 inline-flex px-2 py-0.5 rounded-full bg-accent text-accent-foreground text-[10px] uppercase font-semibold">
                  {g.category}
                </div>
                <button
